...
</script>})  // Tours would be loaded from Supabase in productiononMounted(() => {}  tour.status = tour.status === 'active' ? 'inactive' : 'active'const toggleStatus = (tour) => {}))  avgRating: '4.8'  bookings: tours.value.reduce((sum, t) => sum + t.bookings, 0),  active: tours.value.filter(t => t.status === 'active').length,  total: tours.value.length,const stats = computed(() => ({])  }    image: 'https://images.unsplash.com/photo-1535083783855-76ae62b2914e?w=100&h=100&fit=crop'    status: 'inactive',    bookings: 0,    price: 120,    duration: '1 day',    location: 'Lake Nakuru',    category: 'Wildlife Safari',    name: 'Lake Nakuru Tour',    id: 5,  {  },    image: 'https://images.unsplash.com/photo-1559827260-dc66d52bef19?w=100&h=100&fit=crop'    status: 'active',    bookings: 56,    price: 80,    duration: '1 day',    location: 'Diani',    category: 'Beach Tour',    name: 'Diani Beach Excursion',    id: 4,  {  },    image: 'https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=100&h=100&fit=crop'    status: 'active',    bookings: 32,    price: 200,    duration: '2 days',    location: 'Mount Kenya',    category: 'Adventure',    name: 'Mount Kenya Hike',    id: 3,  {  },    image: 'https://images.unsplash.com/photo-1516426122078-c23e76319801?w=100&h=100&fit=crop'    status: 'active',    bookings: 78,    price: 450,    duration: '3 days',    location: 'Masai Mara',    category: 'Wildlife Safari',    name: 'Masai Mara Safari',    id: 2,  {  },    image: 'https://images.unsplash.com/photo-1611348586804-61bf6c080437?w=100&h=100&fit=crop'    status: 'active',    bookings: 45,    price: 50,    duration: '4 hours',    location: 'Nairobi',    category: 'City Tour',    name: 'Nairobi City Tour',    id: 1,  {const tours = ref([const showAddTour = ref(false)import Button from '@/components/common/Button.vue'import Card from '@/components/common/Card.vue'import MainLayout from '@/components/layout/MainLayout.vue'import { ref, computed, onMounted } from 'vue'<script setup></template>  </MainLayout>    </div>      </div>        </Card>          </div>            </table>              </tbody>                </tr>                  </td>                    </button>                      {{ tour.status === 'active' ? 'Deactivate' : 'Activate' }}                    >                      :class="tour.status === 'active' ? 'text-red-600 hover:text-red-900' : 'text-green-600 hover:text-green-900'"                      @click="toggleStatus(tour)"                     <button                     <button class="text-brand-600 hover:text-brand-900 mr-3">Edit</button>                  <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">                  </td>                    </span>                      {{ tour.status }}                    ]">                      tour.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'                      'px-2 inline-flex text-xs leading-5 font-semibold rounded-full',                    <span :class="[                  <td class="px-6 py-4 whitespace-nowrap">                  </td>                    <div class="text-sm text-gray-900">{{ tour.bookings }}</div>                  <td class="px-6 py-4 whitespace-nowrap">                  </td>                    <div class="text-sm font-semibold text-gray-900">${{ tour.price }}</div>                  <td class="px-6 py-4 whitespace-nowrap">                  </td>                    <div class="text-sm text-gray-900">{{ tour.duration }}</div>                  <td class="px-6 py-4 whitespace-nowrap">                  </td>                    <div class="text-sm text-gray-900">{{ tour.location }}</div>                  <td class="px-6 py-4 whitespace-nowrap">                  </td>                    </div>                      </div>                        <div class="text-sm text-gray-500">{{ tour.category }}</div>                        <div class="text-sm font-medium text-gray-900">{{ tour.name }}</div>                      <div class="ml-4">                      </div>                        <img :src="tour.image" :alt="tour.name" class="h-10 w-10 rounded-lg object-cover">                      <div class="h-10 w-10 flex-shrink-0">                    <div class="flex items-center">                  <td class="px-6 py-4 whitespace-nowrap">                <tr v-for="tour in tours" :key="tour.id">              <tbody class="bg-white divide-y divide-gray-200">              </thead>                </tr>                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Bookings</th>                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Price</th>                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Duration</th>                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Location</th>                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tour</th>                <tr>              <thead class="bg-gray-50">            <table class="min-w-full divide-y divide-gray-200">          <div class="overflow-x-auto">          </div>            <p class="text-sm text-gray-600 mt-1">Manage tour packages and availability</p>            <h2 class="text-xl font-semibold text-gray-900">All Tours</h2>          <div class="mb-6">        <Card>        <!-- Tours Table -->        </div>          </Card>            </div>              </div>                </svg>                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>                <svg class="w-6 h-6 text-yellow-600" fill="currentColor" viewBox="0 0 20 20">              <div class="p-3 bg-yellow-100 rounded-lg">              </div>                <p class="text-2xl font-bold text-yellow-600 mt-1">{{ stats.avgRating }}</p>                <p class="text-sm font-medium text-gray-600">Avg Rating</p>              <div>            <div class="flex items-center justify-between">          <Card>          </Card>            </div>              </div>                </svg>                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path>                <svg class="w-6 h-6 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">              <div class="p-3 bg-purple-100 rounded-lg">              </div>                <p class="text-2xl font-bold text-purple-600 mt-1">{{ stats.bookings }}</p>                <p class="text-sm font-medium text-gray-600">Total Bookings</p>              <div>            <div class="flex items-center justify-between">          <Card>          </Card>            </div>              </div>                </svg>                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>                <svg class="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">              <div class="p-3 bg-green-100 rounded-lg">              </div>                <p class="text-2xl font-bold text-green-600 mt-1">{{ stats.active }}</p>                <p class="text-sm font-medium text-gray-600">Active Tours</p>              <div>            <div class="flex items-center justify-between">          <Card>          </Card>            </div>              </div>                </svg>                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>                <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">              <div class="p-3 bg-blue-100 rounded-lg">              </div>                <p class="text-2xl font-bold text-gray-900 mt-1">{{ stats.total }}</p>                <p class="text-sm font-medium text-gray-600">Total Tours</p>              <div>            <div class="flex items-center justify-between">          <Card>        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">        <!-- Stats Cards -->      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">      </div>        </div>          </div>            </Button>              Add New Tour              </svg>                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>              <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">            <Button variant="primary" @click="showAddTour = true">            </div>              <p class="mt-1 text-sm text-gray-600">Manage and oversee all tour packages</p>              <h1 class="text-3xl font-bold text-gray-900">Tour Management</h1>            <div>          <div class="flex items-center justify-between">        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">      <div class="bg-white border-b border-gray-200 mb-6">      <!-- Admin Header -->    <div class="min-h-screen bg-background-primary">    <div class="container mx-auto px-4 lg:px-8 py-8">
      <!-- Header -->
      <div class="mb-8">
