  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center px-4 py-12">
    <div class="max-w-md w-full">
      ...
      <div class="text-center mb-8 animate-fade-in">
        ...
        <p class="mt-2 text-text-secondary dark:text-gray-400">{{ t('auth.forgotPasswordSubtitle') }}</p>
      </div>
      ...
      <Card padding="lg" class="animate-slide-in">
        <form v-if="!emailSent" @submit.prevent="handleSubmit" class="space-y-5">
          <!-- Email -->
          <div>
            <label class="block text-sm font-medium text-text-brand-600 dark:text-gray-300 mb-2">{{ t('auth.emailAddress') }}</label>
            <Input
              v-model="email"
              type="email"
          ...
          <div class="text-center">
            <router-link to="/login" class="text-sm text-text-secondary dark:text-gray-400 hover:text-brand-600 dark:hover:text-brand-400 inline-flex items-center">
        ...
        <div v-else class="text-center space-y-4">
          ...
          <p class="text-text-secondary dark:text-gray-400">
            ...
            <span class="font-medium text-text-brand-600 dark:text-brand-400">{{ email }}</span>
          </p>
          <Button variant="primary" size="md" full-width @click="router.push('/login')">
            {{ t('auth.backToLogin') }}
          </Button>
          <button @click="resendEmail" class="text-sm text-brand-600 dark:text-brand-400 hover:text-opacity-80">
            {{ t('auth.resendEmail') }}
          </button>
        </div>
      </Card>
