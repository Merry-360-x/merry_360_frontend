  ...
  <form @submit.prevent="submit">
    ...
    <div class="mb-4">
      ...
      <div v-if="form.imageUrl" class="mt-2">
        <img :src="form.imageUrl" alt="Property image" class="w-32 h-32 object-cover rounded" />
      </div>
    </div>
    <button class="btn btn-primary" :disabled="loading">Add Property</button>
  </form>
</template>
<script setup>
import { ref } from 'vue'
...
async function submit() {
  ...
  try {
    const { error } = await supabase.from('properties').insert({ name, description, image_url: imageUrl })
